...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// find current page in coremap,
		pgtbl_entry_t *current = coremap[frame].pte;

		int swp = current->frame & PG_ONSWAP;
		int dirty = current->frame & PG_DIRTY;

		// write current page to swap
		// check if its on swap
		if(swp){
			// on swap
			// only write to swap if its been changed, if its dirty
			if (dirty){
				// write to swap
				int swapcheck = swap_pageout(frame, current->swap_off);
				if(swapcheck == INVALID_SWAP){
					exit(1);
				}else{
					current->swap_off = swapcheck;
					// already on swap, no need to set swap bit
					// set dirty bit to be clean
					current->frame = current->frame & ~PG_DIRTY;
					evict_dirty_count++;
				}
			}else{
			...
			// not on swap, so write page to it
			// only write if its been modified
			if (dirty){
				// set on swap bit
				current->frame = current->frame | PG_ONSWAP;
				...
				// no need to do anything, just evict
				evict_clean_count++;
			}
		...
		// page is on swap so not valid anymore
		current->frame = current->frame & ~PG_VALID;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if((pgdir[idx % PTRS_PER_PGDIR].pde & PG_VALID) == 0){
		pgdir[idx % PTRS_PER_PGDIR] = init_second_level();
	}
	// get the full page table entry for vaddr
	pgtbl_entry_t *pgtbl = (pgtbl_entry_t *)(pgdir[idx % PTRS_PER_PGDIR].pde & PAGE_MASK);
	// Use vaddr to get index into 2nd-level page table and initialize 'p'

	p = pgtbl + PGTBL_INDEX(vaddr);

	// Check if p is valid or not, on swap or not, and handle appropriately
	int validbit = p->frame & PG_VALID;
	int swapbit = p->frame & PG_ONSWAP;
	int frame = -1;


	if (swapbit){
		if (!validbit){
			// not valid and on swap

			// allocate physical frame and fill it
